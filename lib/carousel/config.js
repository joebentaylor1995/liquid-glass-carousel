...
export const LENS = {
  // Which effect the fullscreen pass runs:
  //   glass  — the refracting lens below
  //   bulge  — middle held dead flat, ends swell (uses the edgeClear mask)
  //   sphere — you stand at the middle of the curve and the row wraps around
  //            you. The active card stays flat and the curve leaves its edge
  //            tangentially, so it bends away without a crease at the join.
  mode: "glass", // 'glass' | 'bulge' | 'sphere'

  // bulge
  bulgeCurve: 0.6, // how much taller the panels grow at the ends
  bulgeSpread: 0.18, // how far the row stretches outward
  bulgeShade: 0.12, // light falloff as the surface turns away

  // sphere. Curve brings the ends toward you (so they magnify); squeeze
  // foreshortens them as the surface turns away from you. Both together are
  // what reads as wrapping rather than stretching.
  sphereCurve: 0.5, // how far the ends come toward you
  sphereSqueeze: 0.35, // horizontal foreshortening at the ends
  sphereShade: 0.14, // light falloff with the turn
  // extra flat margin either side of the active card, as a fraction of the
  // screen's half-width. The card itself is always kept flat; this just adds
  // breathing room before the curve starts.
  sphereFlat: 0.03,
