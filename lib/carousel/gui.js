...
export function createCarouselGui(carousel) {
  ...
  // which of the two fullscreen effects runs
  lensFolder.add(LENS, "mode", ["glass", "sphere"]).name("effect");

  const sphereFolder = lensFolder.addFolder("Sphere wrap");
  sphereFolder.add(LENS, "sphereCurve", 0, 2, 0.01).name("curve toward camera");
  sphereFolder.add(LENS, "sphereSpread", 0, 1, 0.01).name("outward stretch");
