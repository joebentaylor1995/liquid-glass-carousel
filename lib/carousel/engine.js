...
export function createCarousel(mount, callbacks = {}) {
  ...
  const lensUniforms = {
    ...
    uMode: { value: LENS.mode === "sphere" ? 1.0 : 0.0 },
    uSphereCurve: { value: LENS.sphereCurve },
    uSphereSpread: { value: LENS.sphereSpread },
  ...
  const lensMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      uniform float uMode;         // 0 = glass lens, 1 = sphere wrap
      uniform float uSphereCurve;  // vertical magnification at the ends
      uniform float uSphereSpread; // how far the row stretches outward
      uniform float uSphereShade;  // falloff as the surface turns away
      ...
      void main(){
        ...
        vec3 outc;
        if (uMode > 0.5) {
          // Sample as if the row were wrapped around a cylinder with its ends
          // rolled toward the camera: closer means magnified, so reading a
          // tighter slice of the buffer at the edges makes those panels grow.
          float m = 1.0 + uSphereCurve * edgeMix;
          float sp = 1.0 + uSphereSpread * edgeMix;
          vec2 uv = vec2(0.5 + (vUv.x - 0.5) / sp, 0.5 + (vUv.y - 0.5) / m);
          outc = texture2D(uTex, uv).rgb;
          // the surface turns as it comes round, so let the light drop with it
          outc *= 1.0 - uSphereShade * edgeMix;
        } else {
  ...
  function tick() {
    ...
    lensUniforms.uMode.value = LENS.mode === "sphere" ? 1.0 : 0.0;
    lensUniforms.uSphereCurve.value = LENS.sphereCurve;
    lensUniforms.uSphereSpread.value = LENS.sphereSpread;
