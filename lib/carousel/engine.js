...
export function createCarousel(mount, callbacks = {}) {
  ...
  // Every panel is the same size, so one shared pair of uniforms feeds all of
  // them. The mask is folded into the stock basic-material shader rather than
  // replacing it, so three keeps owning the map transform and color space.
  const panelUniforms = {
    uSize: { value: new THREE.Vector2(1, 1) }, // panel size in px
    uRadius: { value: CONFIG.RADIUS },
  };
  function roundCorners(mat) {
    mat.onBeforeCompile = (shader) => {
      shader.uniforms.uSize = panelUniforms.uSize;
      shader.uniforms.uRadius = panelUniforms.uRadius;
      // position, not uv: the uv attribute only exists once a map is bound,
      // and the plane is a unit quad so position.xy is already -0.5..0.5
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace(
          ...
          /* glsl */ `
          {
            ...
            float d = sdRoundBox(vQuad * uSize, hs, r);
            diffuseColor.a *= 1.0 - smoothstep(-1.0, 1.0, d); // ~1px feather
          }
          #include <opaque_fragment>`,
        );
    };
  ...
  const lensUniforms = {
    ...
    // horizontal band, measured from screen center in height-fraction units,
    // where the glass is held off entirely (start) and reaches full bite (end)
    uEdgeStart: { value: 0.5 },
    uEdgeEnd: { value: 0.7 },
    // sphere mode: the row curves away through the middle and leans its ends
    // toward the camera, so the outer panels sit closer and magnify
    uMode: { value: 0.0 },
    uBulgeCurve: { value: LENS.bulgeCurve },
    uBulgeSpread: { value: LENS.bulgeSpread },
    uBulgeShade: { value: LENS.bulgeShade },
    uSphereCurve: { value: LENS.sphereCurve },
    uSphereSqueeze: { value: LENS.sphereSqueeze },
    uSphereShade: { value: LENS.sphereShade },
    uFlatHalf: { value: 0.5 },
  };
  const lensMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      uniform float uMode;          // 0 = glass, 1 = bulge, 2 = sphere
      uniform float uBulgeCurve;    // vertical magnification at the ends
      uniform float uBulgeSpread;   // how far the row stretches outward
      uniform float uBulgeShade;    // falloff as the surface turns away
      uniform float uSphereCurve;   // how far the ends come toward you
      uniform float uSphereSqueeze; // foreshortening as the surface turns
      uniform float uSphereShade;   // light falloff with the turn
      uniform float uFlatHalf;      // half-width of the flat active card
      uniform float uEdgeStart;    // |x| where the glass starts to bite
      uniform float uEdgeEnd;      // |x| where it reaches full strength
      ...
      // edgeMix scales the whole effect (displacement AND glow) rather than
      // just its alpha: cross-fading a bent image over an unbent one ghosts,
      // ramping the bend itself reads as glass gradually taking hold.
      vec3 discLens(vec2 center, float aspectCorrect, float edgeMix, out float outA) {
        ...
        float pull = uZoom * 0.30 * (nd * nd) * edgeMix;
        ...
        vec2  rimOff = tangentDir * fluidWave * rimStrength * rScreen * uRimTangential * edgeMix;
        vec2  rimPull = -radialDir * rimStrength * rScreen * uRimInward * edgeMix;
        ...
        vec2  dispDir = offset * uDispersion * 0.004 * rimMask * edgeMix;
        int N = uSamples;
        if (N < 2) N = 2;
        ...
        if (uBlur > 0.01 && blurFade > 0.01) {
          vec2 blurRad = vec2(uBlur) / uRes * blurFade * edgeMix;
          vec3 bcol = vec3(0.0);
          float btw = 0.0;
        ...
        col *= mix(1.0, mix(0.91, 1.0, smoothstep(0.0, 0.38, shapeND)), edgeMix);
        ...
        nova *= uWhiteGlow * (uGlow / 17.0) * 1.15 * edgeMix;
        col += vec3(nova);
        ...
        ring *= uBlueRing * (uGlow / 17.0) * 1.8 * edgeMix;
        if (uShimmer > 0.5) ring *= sin(angle * uShimmerFreq + uTime * uShimmerSpeed) * uShimmerDepth + (1.0 - uShimmerDepth);
        float ringAura = exp(-pow((dC - tR) / (rW * 6.0), 2.0)) * 0.28 * uBlueRing * (uGlow / 17.0) * edgeMix;
        col += uBlueColor * (ring + ringAura);
        // bright border line
        col += vec3(exp(-pow((dC - uRimLinePos) / max(uRimLineWidth, 0.0001), 2.0)) * uRimLine * edgeMix);
      ...
      void main(){
        // Both modes leave the middle alone and only act toward the left and
        // right edges, so the centred card stays flat either way.
        vec2 ec = vUv - 0.5;
        ec.x *= uAspect;
        float edgeMix = smoothstep(uEdgeStart, uEdgeEnd, abs(ec.x));

        vec3 outc;
        if (uMode > 1.5) {
          // Sphere: you are standing at the middle of the curve, so the row
          // wraps around you. Curvature is one continuous parabola across the
          // whole screen (x*x, no edge mask) — masking it would hold the
          // middle flat and put a crease where the warp switched on.
          float x = (vUv.x - 0.5) * 2.0;
          // Flat across the active card, then curving beyond it. Squaring the
          // distance PAST the card edge (rather than masking a curve that
          // starts at the middle) means the bend begins at zero with zero
          // slope — it leaves the flat card tangentially, so there is no
          // crease at the join.
          float t = max(abs(x) - uFlatHalf, 0.0) / max(1.0 - uFlatHalf, 0.001);
          float x2 = t * t;
          // the ends sit closer, so they magnify...
          float m = 1.0 + uSphereCurve * x2;
          // ...while the surface turns away from you, so they foreshorten:
          // reading a WIDER slice horizontally squeezes the panel
          float fs = 1.0 + uSphereSqueeze * x2;
          vec2 uv = vec2(0.5 + (vUv.x - 0.5) * fs, 0.5 + (vUv.y - 0.5) / m);
          // past the buffer's edge there is nothing to sample but page
          outc = (uv.x < 0.0 || uv.x > 1.0)
            ? vec3(1.0)
            : texture2D(uTex, uv).rgb;
          outc *= 1.0 - uSphereShade * x2; // light drops as it turns
        } else if (uMode > 0.5) {
          // Bulge: the earlier take, kept because it reads well — the middle
          // stays dead flat and only the ends swell, via the same edge mask
          // the glass uses.
          float m = 1.0 + uBulgeCurve * edgeMix;
          float sp = 1.0 + uBulgeSpread * edgeMix;
          vec2 uv = vec2(0.5 + (vUv.x - 0.5) / sp, 0.5 + (vUv.y - 0.5) / m);
          outc = texture2D(uTex, uv).rgb;
          outc *= 1.0 - uBulgeShade * edgeMix;
        } else {
          outc = texture2D(uTex, vUv).rgb;  // carousel, untouched
          float a = 0.0;
          vec3 c = discLens(uCenter, uAspect, edgeMix, a);
          outc = mix(outc, c, a);
        }
        ...
        // The FBO holds linear light and this pass writes straight to an sRGB
        // canvas, so the encode has to happen here — three only adds it to its
        // own materials, not to a hand-written ShaderMaterial. Without it
        // every pixel lands ~2.2 gamma too dark.
  ...
  let centeredPanel = null; // the rect nearest screen center
  function layout() {
    panelRects = [];
    centeredPanel = null;
    const half = W / 2;
    ...
    const buffer = wPx;
    panelUniforms.uSize.value.set(wPx, h);
    panelUniforms.uRadius.value = CONFIG.RADIUS;
    pool.forEach((p, poolIdx) => {
      ...
      if (!centeredPanel || Math.abs(centerX) < Math.abs(centeredPanel.centerX))
        centeredPanel = rect;
    });
  }
  ...
  let hoverPanel = false; // raw geometry: the mouse is over some panel
  let hoverSrcIndex = -1; // source index the overlay is showing (-1 = none)
  let overlaySrcIndex = -1; // which image the overlay is pinned to
  ...
  // drag is live -> grab/pointer over a panel -> default. Keyed off hoverPanel
  // (raw geometry), not the overlay's state — every panel is draggable and
  // clickable, the overlay is just fussier about when it shows.
  ...
  function refreshHover() {
    ...
    // The overlay belongs to the centred card alone, and only once the row has
    // come to rest. Hit-testing the pointer every frame while the row moves
    // made it hop from card to card as they slid past the cursor.
    // Compared by source, not pool slot: a wide screen can show two copies of
    // the same image, and which slot the pointer landed in is meaningless.
    // 8px, not 0: the glide's tail is exponential, so waiting for a dead stop
    // costs the better part of a second after the card has visually landed
    const atRest = Math.abs(target - scroll) < 8;
    const onActive =
      hit && centeredPanel && hit.srcIndex === centeredPanel.srcIndex && atRest;

    const next = onActive ? hit.srcIndex : -1;
    if (next !== hoverSrcIndex) {
      hoverSrcIndex = next;
      if (onActive) overlaySrcIndex = next;
      onHoverChange(next);
    }
    updateCursor();

    // Keep the overlay pinned to its image, not to whatever is under the
    // pointer — including on the way out, so the fade travels with the card
    // instead of stranding it mid-screen.
    if (overlayElement && overlaySrcIndex >= 0) {
      let pin = null;
      for (let i = 0; i < panelRects.length; i++) {
        const r = panelRects[i];
        if (r.srcIndex !== overlaySrcIndex) continue;
        // nearest copy to the middle, so it can't latch onto a wrapped twin
        if (!pin || Math.abs(r.centerX) < Math.abs(pin.centerX)) pin = r;
      }
      if (pin) {
        const s = overlayElement.style;
        s.transform = `translate3d(${pin.left}px, ${pin.top}px, 0)`;
        s.width = `${pin.width}px`;
        s.height = `${pin.height}px`;
      }
    }
  }
  ...
  function tick() {
    if (!dragging) {
      ...
      // drag event). The leftover flick is folded into the aim and then spent:
      // left live, `target += velocity` above keeps pushing after the snap has
      // latched and the row lands off-centre with no second snap coming.
      if (
        CONFIG.SNAP &&
        !snapped &&
        performance.now() - lastInput > CONFIG.SNAP_IDLE_MS
      ) {
        // where the momentum would have carried it: v*f + v*f^2 + ...
        const glide = velocity * (CONFIG.FRICTION / (1 - CONFIG.FRICTION));
        velocity = 0;
        target = centerForIndex(nearestIndex(target + glide));
        snapped = true;
      }
    }
    ...
    // The clear band is derived from the panel, not hard-coded: half a card
    // plus edgeClear, in height-fraction units to match the shader's x. Resize
    // the window or change PANEL_H and the active card stays clean regardless.
    const cardHalf = (CONFIG.PANEL_H * CONFIG.ASPECT) / 2 / H;
    lensUniforms.uMode.value =
      LENS.mode === "sphere" ? 2.0 : LENS.mode === "bulge" ? 1.0 : 0.0;
    lensUniforms.uBulgeCurve.value = LENS.bulgeCurve;
    lensUniforms.uBulgeSpread.value = LENS.bulgeSpread;
    lensUniforms.uBulgeShade.value = LENS.bulgeShade;
    lensUniforms.uSphereCurve.value = LENS.sphereCurve;
    lensUniforms.uSphereSqueeze.value = LENS.sphereSqueeze;
    lensUniforms.uSphereShade.value = LENS.sphereShade;
    // the active card's half-width as a fraction of the screen's half-width,
    // so the flat section tracks the panel at any size
    lensUniforms.uFlatHalf.value = Math.min(
      0.95,
      Math.max(0, (CONFIG.PANEL_H * CONFIG.ASPECT) / W + LENS.sphereFlat),
    );
    const edgeStart = cardHalf + LENS.edgeClear;
    lensUniforms.uEdgeStart.value = edgeStart;
    lensUniforms.uEdgeEnd.value = edgeStart + Math.max(LENS.edgeFeather, 0.001);
